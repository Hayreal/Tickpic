...
| 功能 | 说明 |
|------|------|
| 主图 | 基于 SKU 产品图生成电商主图：自动生成英文大标题，并突出核心使用场景。 |
| 对比图 | 每张生成一个场景的一组 Before/After；Before 不展示 SKU，After 可控制是否展示 SKU，并可选择自动、左右或上下布局与效果程度。 |
| 多场景图 | 基于 SKU 产品图生成真实适用场景；提示词可选，可选择单场景、拼图或宫格，SKU 可不露出，默认不添加营销文字。 |

所有 AI 作图功能均采用**单阶段出图流程**：Main Process 在本地将功能 `mainPrompt`、用户 `prompt` 和结构化参数组装为图片执行提示词（`finalPrompt`），并直接调用生成或编辑模型出图。任务产物（`request.json`、`image-instruction.txt`、输出图片等）会保存到用户工作目录，便于复现与排查。
