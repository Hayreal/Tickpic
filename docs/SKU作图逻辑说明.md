...
flowchart LR
    subgraph 图1 新 SKU
        H1[你要换进去的产品]
        H2[锁定：产品身份、包材、标签、品牌、容量]
        H3[不改变：瓶型、比例、材质、透明度]
    end

    subgraph 图2 爆款参考主图
        N1[爆款广告参考]
        N2[继承：卖点、英文文案、使用场景、before/after 逻辑]
        N3[不继承：具体 layout、场景物体、镜头和构图]
    end
```

> 页面上传顺序固定为：**Image 1 = 新 SKU，Image 2 = 爆款参考主图**。模型和执行器沿用同一顺序。
...
A：爆款主图现在按页面上传顺序识别：Image 1 是新 SKU，Image 2 是爆款参考主图；标签模式仍按源图在前、参考图在后。
