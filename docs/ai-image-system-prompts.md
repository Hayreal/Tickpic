...
三个 SKU 功能（`sku_replica`、`sku_variation`、`sku_original`）与套图一样先生成视觉提示词，再由执行提示词组装器合并约束。视觉模型读取 SKU 原图、可选包装设计参考图和用户输入，为每张输出返回英文标签设计计划；组装器再生成完整英文编辑提示词，执行器在写入 `image-instruction.txt` 后才开始出图。三种模式的参考图职责不同：复刻要求高保真复现参考标签；裂变借用同一设计体系但必须采用新的非复刻版式；原创只把参考图作为可选视觉灵感，并创建独立原创版式。所有模式都锁定原 SKU 的构图、机位、透视、容器形态、结构、材质和背景，只改标签承载面。中文产品名、品类、卖点和附加要求会转为自然英文；品牌、容量和型号保持字面值。任何“做成软管/喷雾/罐”等要求只能影响标签品类视觉，不能重做容器。

当 SKU 视觉规划批次中的某一条返回中文或混合语言时，解析器按该条序号替换为英文标签设计兜底计划，继续完成整批规划；不会因为单条规划语言不合规而让多张任务整体失败。最终执行提示词仍由英文校验和约束组装器生成。
...
所有出图在统一执行器中追加通用文字质量约束：不要出现文字乱码，所有可见文字须清晰、可读、拼写正确，不得生成伪字母、随机符号或无意义的假文字。此规则同时覆盖本地组装提示词，以及套图、SKU 和爆款主图经视觉模型生成的每张执行提示词；保存的提示词产物与实际发送内容一致。

产品处理的主图裂变（`main_image_asset_variation`）参照 SKU 爆款主图的反模板约束，禁止小卖点、卖点卡片、功能列表、小图标、圆形/六边形徽章及图标配短文案模块，参考图已有的此类模块也不保留。仅保留主标题和必要副标题，用户卖点用于指导场景和主标题表达；产品本身的包装标签保留。白底产品图补充场景时不再要求添加卖点卡片或图标。
