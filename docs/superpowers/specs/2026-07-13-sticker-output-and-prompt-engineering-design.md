...
4. 移除通用比例预设，只保留一个产品比例控件及其产品预设和自定义入口。
...
界面只提供一个“产品比例”控件，选项包括：

- 自动/不指定。
- `21:5`（罐子）、`21:10`（高罐子）、`9:12`（瓶装）。
- 自定义比例入口，支持正整数或正小数，例如 `7:10`、`2.5:1`。

不再显示 `1:1`、`3:2`、`16:9` 等通用比例预设；需要这些比例时使用自定义入口。界面虽然使用“产品比例”这一业务名称，提交给后端的唯一比例真值仍是 `aspectRatio`，不再同时提交相互竞争的 `aspectRatio` 和 `productRatio`。
...
产品比例下拉展示产品预设和“自定义比例”，不展示通用比例。选择自定义后，在下拉中输入比例宽和比例高。控件下方实时显示最终像素，例如：
...
- 旧任务若 `aspectRatio` 为 `auto` 且包含 `productRatio`，将有效产品比例迁移为唯一的产品比例选择值。
...
- 基础参数显示产品比例、清晰度和生成数量，不再显示通用图片比例控件。
- 产品比例只展示 `21:5`、`21:10`、`9:12`、自动/不指定和自定义入口。
- 默认清晰度为 1K。
- 自定义比例输入实时显示正确像素。
- 非法比例显示就地错误并禁止生成。
...
- 用户只需要设置一个产品比例，不再面对图片比例与产品比例两个控件的冲突。
- 默认 1K，可切换 2K，所有计算尺寸的宽高都是 16 的倍数。
- `1:1 + 1K` 稳定得到目标尺寸 `1024x1024`。
- 产品预设和自定义比例在界面、请求、执行计划和协议载荷中保持一致。
