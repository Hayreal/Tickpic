...
export function buildSkuVisionSystemPrompt(feature: ImageFeature): string {
  ...
  return [
    ...
    ...buildPlannerModeGuidance(feature),
...
export function buildSkuVisionUserText(request: ImageTaskRequest, count: number): string {
  ...
  const imageRoles = (request.images ?? []).map((image, index) => ({
    ...
    purpose: image.role === 'source'
      ...
      : request.feature === 'sku_replica'
        ? 'exact label artwork reference; never a container-form reference'
        : request.feature === 'sku_variation'
          ? 'label design-system reference for a new layout; never a container-form reference'
          : 'optional visual inspiration for an independent label; never a container-form reference',
  }));
...
> = {
  ...
  sku_original: [
    'Use a distinct independent original label layout, hierarchy, and typography scale; if a reference exists, borrow only its loose visual mood.',
    'Apply a different composition structure, color blocking, and decorative motif placement from the other outputs without reproducing the reference layout.',
    'Shift the headline treatment, badge zones, and supporting graphics while keeping the same product name and optional reference inspiration only.',
  ],
};
